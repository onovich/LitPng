...
- 保存 presets（已完成：浏览器本地持久化，支持应用、同名覆盖、删除、损坏数据隔离及最多 20 项限制）。
- 高级重命名模板。
- 高级裁剪 preset。
- 历史任务。
- 批量报告。
...
- 保留文件夹结构（已完成：目录导入、ZIP 层级保留、同目录去重及路径穿越防护）。
- 高级命名模板（已完成：支持可编辑模板及 original/index/prefix/suffix/folder/date token）。
- Shopify / WordPress / Open Graph presets（已完成：提供可撤销为自定义设置的平台参数，Open Graph 使用真实 1200×630 比例裁剪）。
- 压缩报告（已完成：可下载 UTF-8 CSV，包含体积、节省率、耗时、质量、目标达成情况及错误）。
- 对比预览（已完成：Worker 生成小尺寸原图/实际编码结果，支持键盘操作的并排对比）。

### P2：付费功能候选

- 保存 presets（已完成：浏览器本地保存、应用、同名覆盖与删除）。
- 历史任务（已完成本地摘要版：默认关闭、可选记录最近 20 次统计与参数、复用参数、单条删除和清空；不保存图片，不能恢复下载）。
- 大批量队列（已完成基础版：串行解码、单 Worker 调度、暂停/继续/停止、50 条分页、结果清理与故障超时恢复；非无限内存或磁盘流式队列）。
- 批量生成多尺寸。
- `srcset` 生成。
- 桌面版。
- CLI。
