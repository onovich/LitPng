...
- 确认技术授权和基本压缩方案。
- 做出可以公开预告的产品叙事。
- 固定 Phase 0 架构约束，确保后续功能增长不破坏性能、依赖顺序和可读性。
...
- 第三方库许可证清单。
- MVP scope 锁定。
- `docs/phase0-architecture.md`，作为后续命名、目录、依赖方向、控制流和代码风格的强约束。
...
- WebP 输出。
- 目标体积压缩（已完成：质量上限起步、最多 6 次二分尝试，不可达时返回最接近结果并提示）。
- 保留文件夹结构。
- 高级命名模板。
